...
error_reporting(E_ALL);
    ...
    $result['shopPosts'] = array();
        $db = DB::transact_db("SELECT cp.*, CONCAT(c.client_FName, ' ', c.client_MidName, ' ', c.client_LName) AS name, c.client_Address, c.client_Contact, c.client_Photo from client_post cp, laundry_client c where cp.client_ID = c.client_ID",
								array(),
								"SELECT"
                            );
        if(count($db) > 0) {
